...
describe("User model", () => {
  ...
  it("should retrieve only approved permissions through the corresponding method", async () => {
    const { User, Permission } = db.getModels();
    ...
    const approvedPermissions = await user.getApprovedPermissions();

    expect(approvedPermissions.length).toEqual(1);
  ...
  it("should return a list of groups for which the user is approved", async () => {
    ...
    await user.addUserGroup(group1);
    await user.addUserGroup(group2, { through: { approved: true }});

    const groups = await user.getUserGroups();
    expect(groups.length).toEqual(2);

    const approvedGroups = await user.getApprovedUserGroups();
    expect(approvedGroups.length).toEqual(1);
  ...
  it("should have automatically generated association methods for the Post model through Favourite", async () => {
    const { User } = db.getModels();
    const user = await User.create({ username: "test" });
